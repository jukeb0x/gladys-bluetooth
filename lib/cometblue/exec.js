...
const handleHeatingPeriod = require('./handleHeatingPeriod.js');
...
module.exports = {
  ...
  exec: function (peripheral, characteristics, type, value, deviceInfo) {
    return handleHeatingPeriod(type, value, deviceInfo).then((activate) => {
      if(activate==0){
        var message="Le chauffage est désactivé dans la "+deviceInfo.deviceType.roomName;
        var response = {
        ...
        var message="Le chauffage est "+action+" dans la "+deviceInfo.deviceType.roomName;
        var response = {
          label: 'get-thermostat',
          value: value,
          scope: {'%COMET_ANSWER%': message
          ...
          return generateCommand(type, value).then((temp) => {
            ...
            return awoxSend(peripheral, characteristics.get(command.uiid), temp, true).then(() => {
              ...
              return meshFeedback(deviceState,deviceInfo).then(() => {
                ...
                var message="La consigne est de "+value+' '+deviceInfo.deviceType.unit+' dans la '+deviceInfo.deviceType.roomName;
                var response = {
                  label: 'get-thermostat',
                  value: value,
                  scope: {'%COMET_ANSWER%': message
          ...
          return awoxRead(peripheral,characteristics.get(command.uiid)).then((data) => {
            ...
            return decodeAnswer(type,data).then((dataProcessed) => {
              ...
              return meshFeedback(deviceState,deviceInfo).then(() => {
                ...
                var message="Elle est de "+dataProcessed+' '+deviceInfo.deviceType.unit+' dans la '+deviceInfo.deviceType.roomName;
                var response = {
                  label: 'get-thermostat',
                  value: dataProcessed,
                  scope: {'%COMET_ANSWER%': message
