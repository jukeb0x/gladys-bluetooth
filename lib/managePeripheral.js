...
function generateDevice(macAddress, deviceValue) {
  ...
  if(deviceValue !== null && deviceValue !== ''){
    const deviceNameAwox = (deviceValue || '').toString('utf-8').replace('\u0000', '');
    ...
    const deviceNameSwicthBot = 'switchbot_'+macAddress;
     gladysDevice = {
      name: deviceName,
      identifier: macAddress,
      service: 'bluetooth',
  ...
  return Promise.resolve({
    device: gladysDevice,
    remote: remoteDevice,
    types: generateDeviceTypes(remoteDevice, colorDevice, meshDevice)
  });
}

function generateDeviceTypes(remoteDevice, colorDevice, meshDevice) {
  const types = [];
  if (remoteDevice) {
    // TODO battery level
  } else {
