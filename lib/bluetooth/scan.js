      ...
      if (filtered && macAddresses.size === 0) {
        ...
        if (!shared.scanning) {
          noble.startScanning([], false, function () {
            console.log('Bluetooth module: Scanning bluetooth devices');
  ...
  var discover = function (peripheral) {
    if (macAddresses && macAddresses.size !== 0) {
      if (macAddresses.has(peripheral.address)) {
        console.log('Bluetooth module: Detected asked peripheral ' + peripheral.address);
        peripherals.set(peripheral.address, peripheral);
        macAddresses.delete(peripheral.address);
        shared.scanForNb--;

        if (shared.scanForNb === 0) {
          console.log('Bluetooth module: All asked peripherals have been found');
          stop();
      ...
      console.log('Bluetooth module: Detected peripheral ' + peripheral.address);
      peripherals.set(peripheral.address, peripheral);
    }
  };
